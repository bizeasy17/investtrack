...
def mark_tupo_yali_listed(freq, ts_code_list=[]):
    ...
    2. 价格高于前高
    查找算法
    1. 查询获得所有顶部卖点（高点？）index 还是？
    2. 循环每两个顶部之间所有斜率 > 0（slope > 0)的交易点
    1. 查询所有斜率（slope）> 0 的交易点
    2. 循环所有高点
        2.1 在给定的时间段periods = [10, 20, 30, 50, 80]内，向前（day+）搜索指定时间段内高于高点的交易，
        2.2 计算斜率slope（5个交易周期？）
        2.3 如果斜率>0，（向上穿越），价格波动在某一范围内（3%，5%，8%？），则当前底部买点就可标记为支撑位
    '''
    print(ts_code_list)
    # end_date = date.today()
