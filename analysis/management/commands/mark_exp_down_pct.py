...
from analysis.strategy_test_pct_down import test_exp_down_pct
        ...
        if strategy_code is not None and freq is not None:
            if ts_code is not None:
                ...
                if ts_code_list is not None and len(ts_code_list) >= 1:
                    # print(ts_code_list)
                    test_exp_down_pct(strategy_code, ts_code_list, test_freq=freq)
            else:
                test_exp_down_pct(strategy_code, test_freq=freq)
        else:
