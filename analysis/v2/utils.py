...
def mark_slope(df, df_slc, ts_code, day_offset=2, ma_freq='25'):
    # df.loc[:int(ma_freq)-1, 'ma'+ma_freq+"_slope"] = np.nan
    # col='ma' + ma_freq, slope_col='ma'+ma_freq+'_slope', 
    print('mark slope')
    try:
        for index, row in df_slc.iterrows():
            df_let = df_slc[['ma' + ma_freq]].iloc[index -
            ...
            df.loc[index, 'ma'+ma_freq+'_slope'] = round(slope, 3) if slope is not np.nan else np.nan
    except Exception as e:
        print(e)
