    ...
    try:
        ...
        if results is not None and len(results) > 0:
            ...
            for target_pct in target_pct_list:
                ...
                qtiles = df[target_pct].quantile([0.1, 0.25, 0.5, 0.75, 0.9])
    ...
    try:
        ...
        for test_period in test_period_list:
            ...
            if results is not None and len(results) > 0:
                ...
                high_quantile_list.append(round(df.stage_high_pct.max(), 3))
                high_quantile_list.append(round(df.stage_high_pct.min(), 3))
                high_quantile_list.append(round(df.stage_high_pct.mean(), 3))
        ...
        if len(updown_qtiles_list) > 0:
            StrategyUpDownTestQuantiles.objects.bulk_create(updown_qtiles_list)
        print(' updown pct on start - ' + strategy_code + '/' + ts_code +
              ',' + datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    except Exception as err:
        logger.error(err)
        ...
        for test_period in test_period_list:
            ...
            if results is not None and len(results) > 0:
                for quantile in quantile_list:
                    df = pd.DataFrame(results.values(quantile, 'ts_code', 'stock_name'))
                    sorted_df = df.sort_values(by=quantile, ascending=False, ignore_index=True)
        ...
        for target_pct in target_pct_list:
            ...
            if results is not None and len(results) > 0:
                for quantile in quantile_list:
                    ...
                    sorted_df = valid_df.sort_values(by=quantile, ignore_index=True)
