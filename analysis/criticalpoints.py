...
from analysis.utils import (get_analysis_task,
...
def handle_ma(ts_code, freq='D', ma_freq='25', slope_offset=2):
    ...
    if ts_code is None:
        ...
        if ready2proceed('junxian'+ma_freq +
            ...
            init_eventlog('MARK_CP', exec_date,  'junxian'+ma_freq +
                          '_bs', freq=freq)
            process_rma_cp(ts_code, freq, ma_freq,
                               slope_offset)
            set_event_completed('MARK_CP', exec_date, 'junxian'+ma_freq +
                                '_bs', freq=freq)
    else:
        process_rma_cp(ts_code, freq, ma_freq, slope_offset)


def process_rma_cp(ts_codes, freq='D', ma_freq='25', slope_offset=2):
    start_date = None
    end_date = None
    today = date.today()
        ...
        for listed_company in listed_companies:
            ...
            if tasks is not None and len(tasks) > 0:
                for task in tasks:
                    ...
                    calculate_rma(listed_company.ts_code, listed_company.asset, start_date,
...
def calculate_rma(ts_code, asset, start_date, end_date, atype='1', freq='D', ma_freq='25', price_chg_pct=0.03, slope_deg=0.05241, slope_offset=2, version='v2', done_by='system'):
    '''
    目标：
    参数化分析均线，可能对结果有影响的参数
    - 价格变化
...
def mark_zhicheng(price_chg_pct, df_slc, ma_freq,):
    '''
    1. 收盘 > MA && 开盘 > MA
    2. |最低价 - MA| < delta
    '''
...
def mark_tupo(price_chg_pct, df_slc, ma_freq, ):
    '''
    1. 收盘 > MA && 开盘 < MA
    2. 收盘 - MA / MA >= delta
    '''
...
def mark_diepo(price_chg_pct, df_slc, ma_freq, ):
    '''
    1. 收盘 < MA && 开盘 > MA
    2. MA - 收盘 / MA >= delta
    '''
...
def mark_yali(price_chg_pct, df_slc, ma_freq,):
    '''
    1. 收盘 < MA && 开盘 < MA 
