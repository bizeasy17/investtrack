            ...
            for company in companies:
                try:
                    ...
                    if db is not None:
                        ...
                        holders = CompanyTop10FloatHolders.objects.filter(company=company, end_date=company.top10_holder_date)
                        if len(holders) > 0:
                            ...
                            try:
                                ...
                                filter = CompanyTop10FloatHoldersFilter(
                                    company=company, ts_code=company.ts_code, announce_date=ann_date, end_date=company.top10_holder_date, hold_pct=hold_pct)
                                filter.save()
                                print(company.ts_code + ' top10 holder filter created. ' + today.strftime('%Y%m%d HH:MM:SS'))
                    else:
